...
class ServiceContracts_ExportPDF_View extends Vtiger_Index_View {

	function __construct() {
		parent::__construct();
		$this->exposeMethod('exportPDFPopup');
		$this->exposeMethod('downloadPDFFile');
	}
	...
	function exportPDFPopup(Vtiger_Request $request) {
		...
		$mpdf->Output($root_directory.$label.'.pdf','F');

		$viewer = $this->getViewer($request);
		$moduleName = $request->getModule();
		
		$viewer->assign('MODULE', $moduleName);
		$viewer->assign('filepath', $site_URL.$label.'.pdf');
		echo $viewer->view('PreviewPDFTemplate.tpl', $moduleName, true);
	}

	function downloadPDFFile(Vtiger_Request $request) {
		$record = $request->get('record');
		...
		header("Location: $site_URL.$label.'.pdf'");
	}
}
